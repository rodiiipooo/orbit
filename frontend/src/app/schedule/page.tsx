      ...
      <div className="flex-1 p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-white/50 text-sm mt-1">Plan and publish across all platforms</p>
          </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ALL_PLATFORMS.map(p => (
                    <button
                      ...
                      className={cn("btn-secondary text-xs px-3 py-1.5 capitalize", selectedPlatforms.includes(p) && "bg-indigo-600 text-white")}
                    >
                      {p}
                    </button>
                  ))}
        ...
        <div className="card">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {posts.map((p: any) => (
                <div key={p.id} className="flex items-start gap-4 p-4 bg-white/[0.04] rounded-lg">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-white/50">
                      ...
                      {p.ai_generated && <span className="text-indigo-400">AI</span>}
                    </div>
                  </div>
                  <span className={cn("badge flex-shrink-0", STATUS_STYLES[p.status])}>{p.status}</span>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    {p.status === "draft" && (
                      <button onClick={() => handlePublishNow(p.id)} className="p-1.5 hover:bg-white/[0.06] rounded transition-colors" title="Publish now">
                        <Send className="w-4 h-4 text-indigo-400" />
                      </button>
                    )}
                    <button onClick={() => handleDelete(p.id)} className="p-1.5 hover:bg-white/[0.06] rounded transition-colors">
