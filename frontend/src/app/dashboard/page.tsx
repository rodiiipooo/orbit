  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
      <div>
        ...
        <p className="text-white/40 text-sm mt-1">Last 30 days overview</p>
      </div>
        ...
        <div className="card lg:col-span-2">
          ...
          {platformData.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 text-white/30">
              ...
              <Link href="/platforms" className="text-indigo-400 text-sm mt-2 hover:underline">
                Connect platforms →
              </Link>
            </div>
          )}
              ...
              {platforms.map((p: any) => (
                <div key={p.id} className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-white/30">@{p.platform_username}</div>
                  </div>
      ...
      {insights.length > 0 && (
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/analytics" className="text-sm text-indigo-400 hover:underline flex items-center gap-1">
              View all <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="space-y-3">
            {insights.map((ins: any) => (
              <div key={ins.treatment} className="flex items-start gap-4 p-3 bg-white/[0.04] rounded-lg">
                <div className={`badge mt-0.5 ${ins.significant ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-white/[0.06] text-white/45"}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-white/45 mt-0.5 leading-relaxed line-clamp-2">
                    {ins.interpretation}
                  </div>
                </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/schedule" className="text-sm text-indigo-400 hover:underline">View all →</Link>
        </div>
        {posts.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-white/30 text-sm mb-3">No published posts yet.</p>
            <Link href="/content" className="btn-primary text-sm">Create your first post</Link>
          </div>
        ) : (
          <div className="space-y-3">
            {posts.map((p: any) => (
              <div key={p.id} className="flex items-start gap-3 p-3 bg-white/[0.04] rounded-lg">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white line-clamp-2">{p.body}</p>
                  <div className="flex items-center gap-2 mt-1.5">
                    {p.platforms.map((pl: string) => (
                      <span key={pl} className="text-xs text-white/30 capitalize">{pl}</span>
                    ))}
                  </div>
                </div>
                {p.ai_generated && (
                  <span className="badge bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 flex-shrink-0">AI</span>
                )}
              </div>
            ))}
          </div>
